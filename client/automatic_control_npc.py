...
class World(object):
    ...
    def restart(self, args):
        ...
        for i in range(args.num):
            ...
            #     spawn_point.rotation.pitch = 0.0
            #     self.destroy()
            #     self.players[i] = self.world.try_spawn_actor(blueprint, spawn_point)
            #     self.modify_vehicle_physics(self.players[i])
         # Get a random blueprint.
            blueprint = random.choice(self.world.get_blueprint_library().filter(self._actor_filter))
            blueprint.set_attribute('role_name', 'hero')
            if blueprint.has_attribute('color'):
...
def game_loop(args):
    ...
    try:
        ...
        agents = [None] * args.num
        for i in range(args.num):
            agents[i] = BehaviorAgent(world.players[i], behavior=args.behavior)
        ...
        for i in range(args.num):
            destination = random.choice(spawn_points).location
            agents[i].set_destination(destination)
        ...
        while True:
            ...
            for i in range(args.num):
                if agents[i].done():
                    # if args.loop:
                    agents[i].set_destination(random.choice(spawn_points).location)
                    world.hud.notification("The target has been reached, searching for another target", seconds=4.0)
                    print("The target has been reached, searching for another target")
                    # else:
                    #     print("The target has been reached, stopping the simulation")
...
def main():
    ...
    argparser.add_argument(
        '-n', '--num',
        help='The number of cars (default: 10)',
        default=10,
        type=int)
